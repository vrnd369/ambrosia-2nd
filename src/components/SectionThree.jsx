...
function SectionThree() {
  ...
  // Portal: render fixed navbar outside section-three to avoid iOS overflow:hidden clipping
  const usePortalForDesktop = navMode === 'fixed' && !isMobile;
  const usePortalForMobile = isMobile;
  ...
  return (
    <section className="section-three" ref={sectionRef}>
      ...
      {/* Portal: fixed navbar outside section-three to avoid iOS overflow clipping */}
      {usePortalForDesktop && createPortal(desktopNav, document.body)}
      {usePortalForMobile && createPortal(mobileNav, document.body)}
      ...
      <div className="s3-navbar-wrapper" style={navMode === 'fixed' ? { height: placeholderH } : undefined}>
        {/* DESKTOP NAV — in-flow when static */}
        {!usePortalForDesktop && desktopNav}
        {/* MOBILE NAV — in wrapper only on desktop (display:none); on mobile it's in portal */}
        {!usePortalForMobile && mobileNav}
      </div>
